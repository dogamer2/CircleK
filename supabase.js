...
// ---------- CONFIG ----------
const SUPABASE_URL = "[https://hbqermjuktzmtymnzlda.supabase.co](https://hbqermjuktzmtymnzlda.supabase.co)";
...
.on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'totals' }, payload => callback(payload))
.subscribe();
...
.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'bug_reports' }, payload => callback(payload))
.subscribe();
return () => supabase.removeChannel(chan);
} catch (err) {
...
.on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'status_reports' }, payload => callback(payload))
.subscribe();
return () => supabase.removeChannel(chan);
} catch (err) {
